...
int loadCards(int val[CARDS_HAND][CARDS], char col[CARDS_HAND][CARDS]) {
    ...
    for (int i = 0; i < 9; ++i) {
        if (scanf(" %1c%1c%c", &d, &c, &n) == EOF) return 0;
        if (c != 'h' && c != 'd' && c != 's' && c != 'c') {
...
char match_result(int val1[], char col1[], int val2[], char col2[]) {
    ...
    if ((tok1 != 0 && pair1 != 0) || (tok2 != 0 && pair2 != 0)) {
        if (tok1 == tok2) {
            if (pair1 == pair2) return 'D';
            if (pair1 > pair2) return 'W';
            else return 'L';

        }
        if (tok1 > tok2) return 'W';
        else return 'L';
    }
    ...
    if (flush1 != 'n') return 'W';
    if (flush2 != 'n') return 'L';
    ...
    if (pair1 != 0 && pair2 != 0) {
        ...
        if (spair1 == 0 && spair2 == 0) {
            if (pair1 == pair2) return highCard(val1, val2, 3, pair1, spair1);
            if (pair1 > pair2) return 'W';
            else return 'L';
        }
        if (spair1 != 0 && spair2 != 0) {

            if (pair1 == pair2 && spair1 == spair2) return highCard(val1, val2, 1, pair1, spair1);
            if (pair1 == pair2 && spair1 > spair2) return 'W';
            if (pair1 == pair2 && spair1 < spair2) return 'L';
            if (pair1 > pair2) return 'W';
            else return 'L';
        }
        if (spair1 != 0) return 'W';
        return 'L';
    }
    if (pair1 != 0) return 'W';
    if (pair2 != 0) return 'L';
...
int duplicates(int val[CARDS], char col[CARDS]) {
    for (int i = 0; i < CARDS; ++i) {
        for (int j = i + 1; j < CARDS; ++j) {
            if (val[i] == val[j] && col[i] == col[j]) return 1;
        }
    }
    return 0;
}
...
char highCard(const int val1[], const int val2[], int n, int ignore, int ignore2) {
    ...
    while (counter < n && i1 >= 0 && i2 >= 0) {
        if (val1[i1] == ignore || val2[i2] == ignore || val1[i1] == ignore2 || val2[i2] == ignore2) {
            if (val1[i1] == ignore || val1[i1] == ignore2) i1--;
            if (val2[i2] == ignore || val2[i2] == ignore2) i2--;

            continue;
        }
        if (val1[i1] > val2[i2]) return 'W';
        if (val1[i1] < val2[i2]) {
            return 'L';
        }
        counter++;
...
int four_of_kind(int val[]) {
    for (int i = CARDS - 1; i > 2; --i) {
        if (val[i] == val[i - 1] && val[i] == val[i - 2] && val[i] == val[i - 3]) return val[i];
    }
    return 0;
}
...
int three_of_kind(int val[]) {
    for (int i = CARDS - 1; i > 1; --i) {
        if (val[i] == val[i - 1] && val[i] == val[i - 2]) return val[i];
    }
    return 0;
}
...
int pair(int val[], int ignore) {
    for (int i = CARDS - 1; i > 0; --i) {
        if (val[i] != ignore && val[i] == val[i - 1]) return val[i];
    }
    return 0;
}
...
int straightflush(int val[], char col[]) {
    ...
    int counter = 0;
    if (val[6] == 14 && val[0] == 2 && col[6] == col[0]) counter++;
    for (int i = 0; i < CARDS - 1; ++i) {
        if (val[i] == val[i + 1] && col[i] != color) continue;
        if (val[i] == val[i + 1] - 1 && col[i] == color && col[i + 1] == color) counter++;
        else {
            if (counter >= 4) return val[i];
            else counter = 0;
        }
    }
    if (counter >= 4) return val[CARDS];
    return 0;
}

int straight(int val[]) {
    int counter = 0;
    if (val[6] == 14 && val[0] == 2) counter++;

    for (int i = 0; i < CARDS; ++i) {
        if (val[i] == val[i + 1]) continue;
        if (val[i] == val[i + 1] - 1) counter++;
        else {
            if (counter >= 4) return val[i];
            counter = 0;
        }
...
char most_color(const char colors[]) {
    ...
    for (int i = 0; i < CARDS; ++i) {
        if (colors[i] == 'h') counter[0]++;
        if (colors[i] == 'd') counter[1]++;
        if (colors[i] == 's') counter[2]++;
        if (colors[i] == 'c') counter[3]++;
    }
    for (int j = 0; j < 4; ++j) {
        if (counter[j] >= 5)
            switch (j) {
